module Performance
  class ChangeStateProd
    ...
    def self.run_tests(params)
      ...
      accounts.each do | account |
        Concurrent::Promise.execute do
          perform_change_state(account[:subdomain], account[:api_key], num_docs, account[:sequence_id])
        end
      end
    end
    ...
    def self.perform_change_state(subdomain, api_key, num_docs, sequence_id)

      invoices = create_documents(subdomain, api_key, num_docs, sequence_id)
      ...
      invoices.each do | invoice_id |
        ...
        rest_endpoint = change_state_endpoint(subdomain, api_key, 'production', invoice_id)

        Rails.logger.info "\nCalling #{rest_endpoint}"
    ...
    def self.create_documents(subdomain = 'bodesvermelhos', api_key = '5312d0f8f4a20479eb8ae1f44fc973086da07978', num_docs = 1,
                       sequence_id)
      ids = []
      ...
      num_docs.times do
        ...
        rest_endpoint = create_endpoint(subdomain, api_key)
        ...
        response_id = JSON.parse(response)["invoice_receipt"]["id"]
        ids << response_id
    ...
    def self.create_endpoint(subdomain, api_key, env = 'production')
      if env == 'staging'
        "http://#{subdomain}.obiwan.rupeal.com/invoice_receipts.json?api_key=#{api_key}"
      elsif env == 'development'
        "http://localhost:3001/invoice_receipts.json?api_key=#{api_key}"
      elsif env == 'production'
        "https://#{subdomain}.app.invoicexpress.com/invoice_receipts.json?api_key=#{api_key}"
      end
    end

    def self.change_state_endpoint(subdomain, api_key, env = 'production', invoice_id)
      if env == 'staging'
        "http://#{subdomain}.obiwan.rupeal.com/invoice_receipts/#{invoice_id}/change-state.json?api_key=#{api_key}"
      elsif env == 'development'
        "http://localhost:3001/invoice_receipts.json?api_key=#{api_key}"
      elsif env == 'production'
        "https://#{subdomain}.app.invoicexpress.com/invoice_receipts/#{invoice_id}/change-state.json?api_key=#{api_key}"
      end
    end
